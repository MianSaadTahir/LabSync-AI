...
if (!allDepsInstalled) {
    ...
    directories.forEach(dir => {
        ...
        if (fs.existsSync(packageJsonPath) && !fs.existsSync(nodeModulesPath)) {
            try {
                execSync('npm install --legacy-peer-deps', { cwd: dirPath, stdio: 'inherit' });
            } catch (e) {
                console.error(`⚠️  ${dir} installation still failing - proceeding anyway`);
            }
        }
    });
}
...
try {
    ...
    console.warn('⚠️  MongoDB not found in PATH. Please ensure MongoDB is installed and running!');
}
...
if (geminiKey1 && geminiKey2 && geminiKey3) {
    ...
    console.log('   ⚠️  Gemini API Keys: Only 1 key configured (2 more recommended)');
} else {
    console.log('   ⚠️  Gemini API Keys: Not configured - AI features will be limited');
    console.log('      Get keys from: https://makersuite.google.com/app/apikey');
}

// Check for Telegram Bot Token
if (backendEnvContent.includes('TELEGRAM_BOT_TOKEN=') && !backendEnvContent.includes('YOUR_TELEGRAM_BOT_TOKEN')) {
    console.log('   ✅ Telegram Bot: Configured');
} else {
    console.log('   ℹ️  Telegram Bot: Optional - configure in backend/.env if needed');
}

// Check MongoDB
console.log('   ✅ MongoDB: Ready at mongodb://localhost:27017/labsync-ai');
console.log('   ✅ MCP Servers: Enabled for Budget Design & Meeting Extraction');

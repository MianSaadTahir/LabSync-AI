...
class GeminiKeyManager {
    ...
    for (const keyName of keyNames) {
      ...
      if (key && key.length > 0) {
        if (key.startsWith('YOUR_')) {
          console.warn(`[MCP Config] ⚠️  Skipping default placeholder key: ${keyName}`);
          continue;
        }
        this.keys.push(key);
        this.usageCounts.set(this.keys.length - 1, 0);
        console.log(`[MCP Config] ✅ Loaded key: ${keyName} (${key.substring(0, 4)}...${key.substring(key.length - 4)})`);
      }
    }

    if (this.keys.length > 0) {
      console.log(`[MCP Config] 🚀 Ready with ${this.keys.length} valid Gemini API key(s)`);
    } else {
  ...
  getNextKey(): string {
    ...
    this.usageCounts.set(this.currentIndex, (this.usageCounts.get(this.currentIndex) || 0) + 1);
...
export const config = {
  ...
  getGeminiApiKeyForMeeting: (): string => getGeminiKeyManager().getKeyForPurpose('meeting'),
  getGeminiApiKeyForBudget: (): string => getGeminiKeyManager().getKeyForPurpose('budget'),

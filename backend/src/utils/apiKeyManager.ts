...
class GeminiApiKeyManager {
  ...
  private loadKeys(): void {
    ...
    if (this.keys.length === 0) {
      console.warn(
        "[GeminiApiKeyManager] No valid Gemini API keys found in environment"
      );
    } else {
      console.log(
        `[GeminiApiKeyManager] Loaded ${this.keys.length} API key(s) for load balancing`
      );
    }
  }
  ...
  getNextKey(): string {
    if (this.keys.length === 0) {
      throw new Error(
        "No Gemini API keys configured. Set GEMINI_API_KEY in your .env file."
      );
    }
  ...
  reportError(key: string): void {
    ...
    if (keyStats) {
      ...
      if (keyStats.errorCount >= 3) {
        ...
        console.warn(
          `[GeminiApiKeyManager] Key temporarily disabled after ${keyStats.errorCount} errors`
        );
      }
    }
  }

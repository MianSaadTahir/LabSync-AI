...
export class BudgetAllocationService {
  constructor() {
    console.log(
      "[BudgetAllocationService] Initialized - Auto-allocation enabled"
    );
  }
  ...
  async allocateAndSave(budgetId: string): Promise<any[]> {
    try {
      ...
      const message = meeting?.messageId
        ? await Message.findById(meeting.messageId)
        : null;
      ...
      if (existingAllocations.length > 0) {
        console.log(
          `[BudgetAllocationService] Budget ${budgetId} already allocated`
        );

        // Emit event even if already allocated so frontend can navigate
        emitToUpdates(SocketEvents.BUDGET_ALLOCATED, {
          allocations: existingAllocations,
          budgetId: budget._id.toString(),
          projectName: budget.project_name,
          totalAllocated: existingAllocations.reduce(
      ...
      console.log(
        `[BudgetAllocationService] Allocating budget for ${budget.project_name}`
      );
      ...
      console.log(
        `[BudgetAllocationService] ✅ Created ${savedAllocations.length} allocations for budget ${budgetId}`
      );
      ...
      try {
        ...
        console.error(
          "[BudgetAllocationService] Error updating status:",
          updateError
        );
      }
      throw error;

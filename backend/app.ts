  ...
  try {
    ...
    const [
      ...
      Message.countDocuments({
        ...
        module2_status: { $in: ["pending", null] },
      }),
      Budget.countDocuments(),
      Message.countDocuments({
        $or: [{ module1_status: "failed" }, { module2_status: "failed" }],
      }),
    ]);
    ...
    const hasTelegramToken = !!(
      ...
      process.env.TELEGRAM_BOT_TOKEN.length > 10
    );
    ...
    res.json({
      ...
      configuration: {
        ...
        geminiApiKey4: process.env.GEMINI_API_KEY_4 ? "CONFIGURED" : "NOT SET",
        telegramBot: hasTelegramToken ? "CONFIGURED" : "MISSING",
        telegramWebhook: process.env.TELEGRAM_WEBHOOK_URL
          ? "CONFIGURED"
          : "NOT SET",
        mongodb: "CONNECTED",
      },
